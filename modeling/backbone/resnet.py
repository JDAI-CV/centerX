...
class BasicBlock(nn.Module):
    ...
    def __init__(self, cfg, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = get_norm(cfg, planes, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = get_norm(cfg, planes, momentum=BN_MOMENTUM)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, cfg, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1,
                               bias=False)
        self.bn3 = get_norm(cfg, planes * self.expansion,
...
def build_resnet(cfg):
    num_layers = cfg.MODEL.BACKBONE.DEPTH
    pretrain = cfg.MODEL.BACKBONE.PRETRAIN
    block_class, layers = resnet_spec[num_layers]

    model = ResNet(cfg, block_class, layers)
    model.init_weights(num_layers, pretrain)
    return model
